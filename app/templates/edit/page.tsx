...
export default function EditTemplate({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    // Load text elements from database template
    const canvasData = templateData.canvas_data;
    if (!canvasData || !canvasData.textElements) return;
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans text-gray-900 overflow-hidden">
      ...
      <div className="w-96 bg-white border-l border-gray-200 shadow-xl flex flex-col p-6 gap-6 overflow-y-auto">
        ...
        {!loading && textElements.length > 0 ? (
          <div className="space-y-4 flex-1">
            {textElements.map((element) => (
              <div key={element.id} className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                <label className="text-xs font-bold text-gray-600 uppercase tracking-wider mb-2 block">
                  {element.label}
                </label>
                <textarea
                  className="w-full p-3 border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm resize-none transition"
                  rows={2}
                  value={element.text}
          ...
          <div className="flex-1 flex items-center justify-center text-gray-400">
            {loading ? 'Loading...' : 'No text fields available'}
          </div>
        )}
