...
export default function EditTemplateNew({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchTemplate = async () => {
      try {
        ...
        if (result.success) {
          ...
          // Initialize text elements for each page
          if (template.is_multipage && template.pages) {
            setPageTextElements(
              template.pages.map((page) =>
                page.canvasData.textElements
              )
            );
          } else {
            setPageTextElements([
              template.canvas_data.textElements,
            ]);
          }
        } else {
  ...
  const resetToDefault = () => {
    ...
    if (templateData.is_multipage && templateData.pages) {
      ...
      setPageTextElements([
        templateData.canvas_data.textElements,
      ]);
    }
  };
  ...
  const saveAndNext = () => {
    if (!templateData) return;
    // Text is already in pageTextElements; just advance
  ...
  // Prepare pages for MultiPageCanvas - use pageTextElements state for current text values
  const pages = templateData.is_multipage && templateData.pages
  ...
  const visibleTextElements = currentPageTextElements.filter((el) => {
    // Check if locked (you can enhance this logic)
        ...
        <div className="border-b pb-4">
          ...
          {pages.length > 1 && (
            <p className="text-xs text-blue-600 mt-2">
              Multi-page template ({pages.length} pages)
            </p>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          {visibleTextElements.length === 0 ? (
            <div className="text-center text-gray-400 text-sm">
              All fields are locked for this page.
            </div>
          ) : (
              ...
              {visibleTextElements.map((element) => (
                <div key={element.id}>
                  ...
                  <input
                    ...
                    value={element.text}
                    onChange={(e) =>
                      handleTextChange(currentPageIndex, element.id, e.target.value)
                    }
